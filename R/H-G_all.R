...
imp <- function(n_cycle, scenar, metrica){
  ...
  for (dist in distrs){
    ...
    for (n_c in seq(from = 1, to = n_cycle, by = 1)){
      ...
      #rfcont10_10
      #----------------
      #df_fin <- metr_func(X_miss, 'rfcont10', df, metrica, col2, df_fin, dist, scenar)
...
print_res <- function(n_cy, scenar, metrica){
  ...
  #res$estimator <- fct_rev(factor(res$estimator, levels = c('missForest', 'rfcont10', 'micePMM')))
  res$estimator <- fct_rev(factor(res$estimator, levels = c('missForest', 'micePMM')))
...
#     write.csv(res, paste("Normmixs_", metric, "_scen", i,".csv" , sep = ""), row.names = FALSE)
#   }
#   i <- i+1
# }
# 
...
# i <- 2
# for(scenar in list('logr_quadr', 'lr_inter', 'logr_inter')){
#   for (metric in list('nrmse')){
#     print(paste(scenar, "+", metric, sep = ""))
#     res <- print_res(100, scenar, metric)
#     write.csv(res, paste("MNAR_", metric, "_sc", i,".csv" , sep = ""), row.names = FALSE)
#     res
#   }
#   i <- i+1
# }
...
#     print(paste(scenar, "+", metric, sep = ""))
#     res <- print_res(100, scenar, metric)
#     write.csv(res, paste("/Users/ash/Desktop/MICE/R/H-G csv/0.45_miss/", metric, "_sc", i,".csv" , sep = ""), row.names = FALSE)
#     res
#   }
#   i <- i+1
# }

# MNAR_rel_bias_scen1 <- read.csv("/Users/ash/Desktop/MICE/PY/fin_res_csv/MNAR_rel_bias_scen1.csv")
# MNAR_rel_bias_scen1
# colnames(MNAR_rel_bias_scen1)[4] <- 'res' 
# fin_plot <- plott(res = MNAR_rel_bias_scen1, sc_num = 1, metrica = 'rel_bias')
# fin_plot[[1]] #+ xlim(0.6, 1.6)


# i <- 2
# for(scenar in list('logr_quadr', 'logr_inter')){
#   for (metric in list('relb_coef')){
#     print(paste(scenar, "+", metric, sep = ""))
#     res <- print_res(500, scenar, metric)
#     write.csv(res, paste("new",metric, "_scen", i,".csv" , sep = ""), row.names = FALSE)
#   }
#   i <- 4
# }


# ress <- read.csv("/Users/ash/nrmse_scen2.csv")
# fin_plot <- plott(res = ress, sc_num = 4, metrica = 'nrmse')
# fin_plot[[1]] + xlim(0.6, 1.6)
# 
# 
